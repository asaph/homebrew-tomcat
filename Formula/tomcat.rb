  ...
  def install
    ...
    if build.with? 'ssl'
      ...
      if build.with? 'apr'
        ...
        system "#{Formula['openssl'].bin}/openssl req -new -newkey rsa:4096 -nodes -days 365 -x509 -subj \"/C=/ST=/L=/O=/CN=localhost\" -keyout #{libexec}/conf/privkey.pem -out #{libexec}/conf/cacert.pem"
